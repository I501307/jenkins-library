...
func TestMarBuild(t *testing.T) {
	...
	t.Run("Provide default npm registry", func(t *testing.T) {
		...
		utilsMock := newMtaBuildTestUtilsBundle()
		options := mtaBuildOptions{ApplicationName: "myApp", DefaultNpmRegistry: "https://example.org/npm", MtarName: "myName"}
	...
	t.Run("Write yaml file", func(t *testing.T) {
		...
		options := mtaBuildOptions{ApplicationName: "myApp", MtarName: "myName"}
